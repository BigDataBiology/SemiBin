...
def cluster(model,data,device,max_edges,max_node,is_combined,logger,n_sample,contig_length_dict,out,contig_dict,binned_short):
    ...
    with torch.no_grad():
        ...
        embedding_matrix = kneighbors_graph(embedding, n_neighbors=max_edges, mode='distance', p=2, n_jobs=-1).toarray()
        kmer_matrix = kneighbors_graph(train_data_input,n_neighbors=max_edges, mode='distance', p =2, n_jobs=-1).toarray()
