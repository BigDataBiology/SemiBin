...
* `--engine`: device used to train the model (auto/gpu/cpu, auto means if SemiBin detects the gpu, SemiBin will use GPU).
* `--depth-metabat2`: depth file generated by metabat2 (single-sample binning).
 <br/><br/>
...
* `--reference-db-data-dir`, `--processes`, `--minfasta-kbs`, `--recluster`,`--epoches`, `--batch-size`, `--max-node`, `--max-edges`, `--random-seed`, `--ratio`, `--min-len`, `--ml-threshold`, `--no-recluster`, `--orf-finder` and `--engine` are same as for `single_easy_bin`
...
* `-p/--processes/-t/--threads`, `--ratio`, `--min-len`, `--ml-threshold` and `--depth-metabat2` are same as for `single_easy_bin`.
...
* `--epoches`, `--batch-size`, `-p/--processes/-t/--threads`, `--random-seed`, `--ratio`, `--min-len`, `--orf-finder` and `--engine` are the same as for `single_easy_bin`
